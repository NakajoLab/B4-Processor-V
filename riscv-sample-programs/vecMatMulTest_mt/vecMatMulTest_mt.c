...
void _internal_e32_8x8_half_matmul(const int array0[4][8], const int array1[8][8], int target[4][8]) {
  ...
  asm volatile ("vmv.s.x v9, zero");
  int i=0, j=0;
  for(i=0; i<4; i++) {
    asm volatile ("vle32.v v10, (%0)"::"r"(&(array0[i][0])));
    asm volatile ("vle32.v v11, (%0)"::"r"(&(array0[i][4])));
    for(j=0; j<8; j++) {
      asm volatile ("vle32.v v12, (%0)"::"r"(&(array1[j][0])));
      asm volatile ("vle32.v v13, (%0)"::"r"(&(array1[j][4])));
      asm volatile ("vmul.vv v12, v12, v10");
      asm volatile ("vmul.vv v13, v13, v11");
      asm volatile ("vadd.vv v12, v12, v13");
      asm volatile ("vredsum.vs v12, v12, v9");
      asm volatile ("vmv.x.s %0, v12":"=r"(target[i][j]));
    }
  }
  return;
}
...
long main(long loop_count) {
  ...
  int *ptr0 = (int*)0x801002c8;
  int *ptr1 = (int*)0x801001c8;
