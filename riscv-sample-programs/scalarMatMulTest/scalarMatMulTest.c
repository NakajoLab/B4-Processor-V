...
// array1は転置行列
void _e32_8x8_matmul_mt(const int array0[8][8], const int array1[8][8], int target[8][8]) {
  int i=0, j=0, k = 0;
  for(i=0; i<8; i++) {
    for(j=0; j<8; j++) {
      int sum = 0;
...
long main(long loop_count) {
  ...
  int *ptr0 = (int*)0x80100250;
  int *ptr1 = (int*)0x80100150;
  int *vecDestPtr = (int*)0x80100508;
  int *ansPtr = (int*)0x80100400;

  PERFORMANCE_COUNT();
  asm volatile ("fence");
  _e32_8x8_matmul_mt(ptr0, ptr1, vecDestPtr);
  asm volatile ("fence");
  PERFORMANCE_COUNT();
